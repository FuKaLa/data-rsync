...
public class ConnectionPoolManager {
    ...
    private void initDefaultConfigs() {
        ...
        mysqlProps.setProperty("initialSize", "5");
        mysqlProps.setProperty("minIdle", "5");
        mysqlProps.setProperty("maxActive", "20");
        mysqlProps.setProperty("maxWait", "60000");
        mysqlProps.setProperty("timeBetweenEvictionRunsMillis", "60000");
        mysqlProps.setProperty("minEvictableIdleTimeMillis", "300000");
        mysqlProps.setProperty("validationQuery", "SELECT 1 FROM DUAL");
        mysqlProps.setProperty("testOnBorrow", "false");
        mysqlProps.setProperty("testOnReturn", "false");
        mysqlProps.setProperty("testWhileIdle", "true");
        mysqlProps.setProperty("poolPreparedStatements", "true");
        mysqlProps.setProperty("maxPoolPreparedStatementPerConnectionSize", "20");
        ...
        switch (environment) {
            case "dev":
                ...
                if ("mysql".equals(poolType)) {
                    envConfig.setProperty("maxActive", "10");
                }
                break;
            case "test":
                // 测试环境配置
                if ("mysql".equals(poolType)) {
                    envConfig.setProperty("maxActive", "15");
                }
                break;
            case "prod":
                // 生产环境配置
                if ("mysql".equals(poolType)) {
                    envConfig.setProperty("maxActive", "20");
                }
                break;
        }
