    ...
    <el-card shadow="hover">
      <template #header>
        <div class="card-header">
          <span>任务列表</span>
          <el-button type="primary" @click="handleCreate" :loading="creatingTask">创建任务</el-button>
        </div>
      </template>
      <el-table v-if="!loading" :data="tasks" style="width: 100%">
        ...
        <el-table-column prop="lastExecTime" label="最后执行时间" />
        <el-table-column prop="nextExecTime" label="下次执行时间" v-if="hasScheduledTasks" />
        <el-table-column prop="createTime" label="创建时间" />
        <el-table-column label="操作">
          <template #default="scope">
            <el-button size="small" @click="handleDetail(scope.row.id)">详情</el-button>
            <el-button size="small" @click="handleEdit(scope.row)">编辑</el-button>
            <el-button size="small" type="danger" @click="handleDelete(scope.row.id)" :loading="deletingTask === scope.row.id">删除</el-button>
            <el-button size="small" type="primary" @click="handleStart(scope.row.id)" :disabled="scope.row.status === 'RUNNING'" :loading="operatingTask === scope.row.id">启动</el-button>
            <el-button size="small" @click="handlePause(scope.row.id)" :disabled="scope.row.status !== 'RUNNING'" :loading="operatingTask === scope.row.id">暂停</el-button>
            <el-button size="small" @click="handleResume(scope.row.id)" :disabled="scope.row.status !== 'PAUSED'" :loading="operatingTask === scope.row.id">继续</el-button>
            <el-button size="small" @click="handleRollback(scope.row.id)" :loading="operatingTask === scope.row.id">回滚</el-button>
...
const operatingTask = ref<number | null>(null)
...
const loadTaskList = async () => {
  ...
  try {
    ...
    errorMessage.value = '加载任务列表失败: ' + (error.message || '未知错误')
...
const hasScheduledTasks = computed(() => {
  return tasks.value.some(task => task.nextExecTime)
})
...
const handleDelete = async (id: number) => {
  ...
  try {
    ...
    errorMessage.value = '删除任务失败: ' + (error.message || '未知错误')
    showError.value = true
    setTimeout(() => {
      showError.value = false
    }, 3000)
...
const handleStart = async (id: number) => {
  ...
  try {
    ...
    errorMessage.value = '启动任务失败: ' + (error.message || '未知错误')
    showError.value = true
    setTimeout(() => {
      showError.value = false
    }, 3000)
  } finally {
    operatingTask.value = null
  }
}
...
const handlePause = async (id: number) => {
  ...
  try {
    ...
    errorMessage.value = '暂停任务失败: ' + (error.message || '未知错误')
    showError.value = true
    setTimeout(() => {
      showError.value = false
    }, 3000)
...
const handleResume = async (id: number) => {
  ...
  try {
    ...
    errorMessage.value = '继续任务失败: ' + (error.message || '未知错误')
    showError.value = true
    setTimeout(() => {
      showError.value = false
    }, 3000)
...
const handleRollback = async (id: number) => {
  ...
  try {
    ...
    errorMessage.value = '加载任务版本失败: ' + (error.message || '未知错误')
    showError.value = true
    setTimeout(() => {
      showError.value = false
    }, 3000)
...
const confirmRollback = async () => {
  ...
  try {
    ...
    errorMessage.value = '回滚任务失败: ' + (error.message || '未知错误')
    showError.value = true
    setTimeout(() => {
      showError.value = false
    }, 3000)
