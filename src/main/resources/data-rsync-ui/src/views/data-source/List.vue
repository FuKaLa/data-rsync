      ...
      >
        ...
        <div class="card-content">
          <div class="connection-info">
            ...
            <div class="info-item" v-if="dataSource.dataSourceGroup">
              <el-icon class="info-icon"><Grid /></el-icon>
              <span class="info-label">分组:</span>
              <span class="info-value">{{ dataSource.dataSourceGroup }}</span>
            </div>
            <div class="info-item" v-if="dataSource.heartbeatTime">
              <el-icon class="info-icon"><Timer /></el-icon>
              <span class="info-label">心跳耗时:</span>
              <span class="info-value">{{ dataSource.heartbeatTime }}ms</span>
            </div>
            <div class="info-item" v-if="dataSource.failureCount && dataSource.failureCount > 0">
              <el-icon class="info-icon"><Warning /></el-icon>
              <span class="info-label">连续失败:</span>
              <span class="info-value">{{ dataSource.failureCount }}次</span>
            </div>
            <div class="info-item">
        ...
        <div class="card-actions">
          <el-button size="small" class="action-btn view" @click="handleView(dataSource)">
          ...
          <el-button size="small" class="action-btn test" @click="handleTestConnection(dataSource.id)" :loading="testingConnection">
            <el-icon><CircleCheck /></el-icon>
            测试
          </el-button>
          <el-button size="small" class="action-btn diagnose" @click="handleDiagnose(dataSource.id)" :loading="diagnosing">
            <el-icon><Warning /></el-icon>
            诊断
          </el-button>
          <el-button size="small" class="action-btn delete" @click="handleDelete(dataSource.id)" :loading="deletingDataSource">
...
import { Check, Warning, Close, DataAnalysis, Link, UserFilled, Timer, InfoFilled, Edit, Delete, Warning as WarningIcon, CircleCheck, Plus, Search, Loading, DocumentRemove, View, Grid, Calendar } from '@element-plus/icons-vue'
...
// 操作状态
const enablingDataSource = ref(false)
...
const handleView = (row: any) => {
  router.push(`/data-source/detail?id=${row.id}`)
}
...
const handleDelete = async (id: number) => {
  ...
  try {
    ...
    errorMessage.value = '删除数据源失败: ' + (error.message || '未知错误')
...
const handleEnableChange = async (row: any) => {
  enablingDataSource.value = true
  try {
    await dataSourceApi.enable(row.id, row.enabled)
    successMessage.value = row.enabled ? '数据源已启用' : '数据源已禁用'
    showSuccess.value = true
    setTimeout(() => {
      showSuccess.value = false
    }, 3000)
    ...
    errorMessage.value = '修改数据源状态失败: ' + (error.message || '未知错误')
    showError.value = true
    setTimeout(() => {
      showError.value = false
    }, 3000)
  } finally {
    enablingDataSource.value = false
  }
}
...
const handleDiagnose = async (id: number) => {
  ...
  try {
    ...
    errorMessage.value = '诊断数据源失败: ' + (error.message || '未知错误')
    showError.value = true
    setTimeout(() => {
      showError.value = false
    }, 3000)
